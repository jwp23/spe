...
When changes affect UI rendering (canvas drawing, overlay positioning, layout, visual states) — any change to `src/ui/canvas/`, overlay drawing, coordinate math, or visual state transitions — verify visually with the screenshot tool before claiming completion. See `docs/screenshot-tool.md` for prerequisites and the command workflow.
